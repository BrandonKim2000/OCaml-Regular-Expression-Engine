...
let rec regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  match regexp with 
  (* epsilon in the DFA/NFA/REGEX slides *)
  ...
  (* Symbol in the alphabet in the DFA/NFA/REGEX slides *)
  ...
  | Union (r1, r2) -> let s0 = fresh() in let s1 = fresh() in 
    let x = regexp_to_nfa r1 in let y = regexp_to_nfa r2 in {
      sigma = union x.sigma y.sigma;
      qs = union (union x.qs y.qs) [s0; s1];
      q0 = s0;
      fs = [s1];
      delta = union [(s0, None, x.q0); (s0, None, y.q0); (hd x.fs, None, s1); (hd y.fs, None, s1)] (union x.delta y.delta);
    }
  | Concat (r1, r2) -> let x = regexp_to_nfa r1 in let y = regexp_to_nfa r2 in {
    sigma = union x.sigma y.sigma;
    qs = union x.qs y.qs;
    q0 = x.q0;
    fs = y.fs;
    delta = union (union x.delta y.delta) [(hd x.fs, None, y.q0)];
  }
  | Star x -> let x_new = regexp_to_nfa x in let s0 = fresh() in let s1 = fresh() in {
    sigma = x_new.sigma;
    qs = union x_new.qs [s0; s1];
    q0 = s0;
    fs = [s1];
    delta = union x_new.delta [(hd x_new.fs, None, s1); (s0, None, x_new.q0); (s0, None, s1); (s1, None, s0)];
  }
;;
(*****************************************************************)
